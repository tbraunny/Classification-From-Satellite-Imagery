...
def load_model():
    print("Loading model...")
    model = cnn.CNN() # import model from original py file
    checkpoint = torch.load('planesnet_weights.pt' , weights_only=True) # load only the weights at checkpoint
...
def evaluate_scene(model , scene , stride):
    ...
    for h in range(0 , height - 20 , stride):
        print(f"\tEvaluating row {(h // stride) + 1} of {((height - 20) // stride)}")
        ...
        for w in range(0 , width - 20 , stride):
            ...
            if confidence > 0.84: # introduce confidence to lower false positive rate
    ...
    print(f"Scene Evaluated in {(end - start):.3f} seconds")
    plt.savefig('scene_evalution_test.png')
    plt.show()

if __name__ == '__main__':
    num = 1 # specify scene number to test on
    stride = 3 # specify the granularity of evaluations
    model = load_model()
    scene = load_scene(num)
    evaluate_scene(model , scene , stride)