...
class ObjectDetectionModel:
    ...
    def predict(self, data_loader, confidence_threshold=0.5):
        """
        ...
        with torch.no_grad():
            for images, _ in data_loader:
                ...
                for output in outputs:
                    boxes = output['boxes'][output['scores'] >= confidence_threshold].cpu()
                    scores = output['scores'][output['scores'] >= confidence_threshold].cpu()
                    labels = output['labels'][output['scores'] >= confidence_threshold].cpu()
