...
class PlaneDetector:
    ...
    def evaluate_model(self,model, data_loader, coco_gt, output_json="detections.json", confidence_threshold=0.5):
        ...
        model.eval()
        results = []
        image_ids = data_loader.dataset.image_ids  # ensure your dataset has this attribute
...
def main(train=True, test=True, inference=True):
    # Define file paths
    HRPlanes_path_train = 'data/HRPlanes_coco/train/'
    annotation_HRPlanes_path_train = 'data/HRPlanes_coco/train/_annotations.coco.json'

    HRPlanes_path_valid = 'data/HRPlanes_coco/valid/'
    annotation_HRPlanes_path_valid = 'data/HRPlanes_coco/valid/_annotations.coco.json'

    HRPlanes_path_test = 'data/HRPlanes_coco/test/'
    annotation_HRPlanes_path_test = 'data/HRPlanes_coco/test/_annotations.coco.json'

    external_images_path = 'data/'
    ...
    stats = model.evaluate_model(detector.model, test_loader, test_dataset.coco, output_json="output/test_predictions.json", confidence_threshold=0.97)
    print("Evaluation Stats:", stats)

if __name__ == '__main__':
    main(train=True, test=False, inference=False)
